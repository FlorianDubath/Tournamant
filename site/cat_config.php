...
echo'        
       <div class="cont_l">
         ...
         while( $stmt->fetch()){
         echo'  <tr><td>'.$catName.'('.$catShortName.')</td><td>'.$gender.'</td><td>'. (date("Y", strtotime($RegistrationBegin))-$maxa).'-'.(date("Y", strtotime($RegistrationBegin))-$mina).' ('.$mina.'-'.$maxa.' ans)</td><td>'.$duration.' min</td><td>'.$adaptWeight.'</td><td>'.$RegistrationBegin.'</td><td>'.$RegistrationEnd.'</td><td><a class="grdBtn" href="configcat.php?id='.$Id.'">Modifier</a><a class="grdBtn" href="configcat.php?id='.$Id.'&del=1">Supprimer</a></td></tr>';
         }
