...
$stmt = $mysqli->prepare("SELECT Id,EMail,DisplayName,Salt FROM TournamentSiteUser WHERE Id=?");
$stmt->bind_param("i", $user_id);     
$stmt->bind_result($uid, $EMail,$disp,$last,$salt);
$stmt->execute();
$stmt->fetch();
$stmt->close();
  ...
  <div class="f_cont">       
      <div class="cont_l">
        <div class="h">
            ...
            Desiné à :'.$disp.'<br/>
            Valide jusqu\'à :'. date("Y-m-d H:i:s",strtotime(date("Y-m-d H:i:s")." +5 minutes")).'<br/>
            <div id="qrcode"></div>
